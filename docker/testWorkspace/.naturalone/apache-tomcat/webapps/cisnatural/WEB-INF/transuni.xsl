...
<!--  *************  END Global variables  **************** -->
    ...
    <!-- Set intensified attribute - draw text as bold    -->      
        ...
        <xsl:choose>
            <xsl:when test="@type='input'">
                <input type="text" id="Element_{@id}" name="Element_{@id}" data-testtoolid="input_{@col}_{@row}" class="InputField natInput{@forecol}{$FontWeightCol} {$FontWeight} {$TextDecoStyle} {$FontStyle}" style="{$style} {((100 * (@col - 1 + $correct_cols)) div $col_mult_factor)}%;  TOP: {@row*$row_mult_factor+$topoffset}%; POSITION: absolute;" size="{@displen}" maxlength="{@len}" value="{translate(@initstr,$convFrom, $convTo)}" >
                    ...
                    <xsl:if test="@password_field='True'">
            ...
            <xsl:when test="@type='static'">
                <input id="Element_{@id}" type="text" name="Element_{@id}" data-testtoolid="output_{@col}_{@row}" class="OutputField natOutput{@forecol}{$FontWeightCol} {$VariableOrLiteral} {$FontWeight} {$TextDecoStyle} {$FontStyle}" tabindex="-1" readonly="readonly" style="{$style} {((100 * (@col - 1 + $correct_cols)) div $col_mult_factor)}%;  TOP: {@row*$row_mult_factor+$topoffset}%; POSITION: absolute" size="{@displen}" value="{translate(.,$convFrom, $convTo)}">
                    <xsl:if test="@password_field='True'">
                    ...
                    <xsl:if test="not(@readonly)">
                        ...
                        <xsl:attribute name="onclick">handleFocusSwitch(this.id, this)</xsl:attribute>
                        <xsl:attribute name="ondblclick">handleDoubleClick(this.id, null)</xsl:attribute>            
                    </xsl:if>
                </input>
            </xsl:when>
      <!--xsl:otherwise/-->
    ...
    <!--div class="wintitle"><xsl:value-of select='@title'/></div-->
        <xml id="window">
       ...
       <!--nuwinforecol><xsl:value-of select='@forecol'/></nuwinforecol-->
       <!--nuwinbackcol><xsl:value-of select='$defbackcolor'/></nuwinbackcol-->
            </nuwindow>
        </xml>
    </xsl:template>
